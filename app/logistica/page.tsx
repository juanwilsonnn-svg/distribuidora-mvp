...
export default function Logistica() {
  const { pedidos, cambiarEstado } = useApp()

  const listos    = pedidos.filter(p => p.estado === 'listo')
  const enEntrega = pedidos.filter(p => p.estado === 'en_entrega')
    ...
    <div className="page">
      <div className="page-head">
        <div>
          ...
          <div className="page-sub">
            {listos.length} para despachar · {enEntrega.length} en camino
          </div>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{
          ...
          paddingBottom: 10, borderBottom: '2px solid #7e22ce22',
        }}>
          <div style={{
            width: 10, height: 10, borderRadius: '50%', flexShrink: 0,
            background: listos.length > 0 ? '#7e22ce' : 'var(--muted)',
          }} />
          <span style={{ fontWeight: 800, fontSize: 16, color: listos.length > 0 ? '#7e22ce' : 'var(--muted)' }}>
        ...
        {listos.length === 0 ? (
          <div style={{
            ...
            border: '1.5px dashed var(--line)', color: 'var(--muted)', fontSize: 13,
          }}>
            No hay pedidos listos para despachar ahora.
            <br />
            <span style={{ fontSize: 12, marginTop: 4, display: 'block' }}>
              Los pedidos marcados como "listo" en el depósito aparecerán acá.
            </span>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {listos.map(p => (
              <LogCard key={p.id} pedido={p} onCambiar={cambiarEstado} accentColor="#7e22ce" />
            ))}
          </div>
        )}
      </div>
      ...
      <div>
        <div style={{
          display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14,
          paddingBottom: 10, borderBottom: '2px solid var(--amber-b)',
        }}>
          <div style={{
            width: 10, height: 10, borderRadius: '50%', flexShrink: 0,
            background: enEntrega.length > 0 ? 'var(--amber)' : 'var(--muted)',
          }} />
          <span style={{ fontWeight: 800, fontSize: 16, color: enEntrega.length > 0 ? 'var(--amber)' : 'var(--muted)' }}>
            En camino
          </span>
          {enEntrega.length > 0 && (
            <span style={{
        ...
        {enEntrega.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {enEntrega.map(p => (
              <LogCard key={p.id} pedido={p} onCambiar={cambiarEstado} accentColor="var(--amber)" />
            ))}
          </div>
        )}
      </div>
...
function LogCard({ pedido, onCambiar, accentColor }: {
  pedido: Pedido
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '14px 18px 10px', gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--muted)', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {pedido.notas && (
              <span style={{ background: 'var(--amber-s)', color: 'var(--amber)', padding: '1px 8px', borderRadius: 4, fontWeight: 600, fontSize: 12 }}>
                📝 {pedido.notas}
              </span>
            )}
          </div>
      ...
      <div style={{ padding: '0 18px 16px', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        {isListo && (
          <button
            ...
            style={{ background: '#7e22ce', color: '#fff', minWidth: 180 }}
            onClick={() => onCambiar(pedido.id, 'en_entrega')}
          >
            🚚 Enviar a entrega
          </button>
        )}
